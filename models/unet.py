...
class Unet(nn.Module):
    ...
    def forward_with_cond_scale(
        ...
        **kwargs
        ...
        # if remove_parallel_component and cond_scale != 0:
    ...
    def forward(
        ...
        # Ensure x has the right shape for 2D convolutions (batch, channels, height, width)
        if len(x.shape) != 4:
        ...
        # Ensure time tensor is 1D with correct batch size
        if time.shape != (batch,):
            time = time.reshape(batch)
        
        # Ensure classes tensor is 1D with correct batch size
        if classes.shape != (batch,):
            classes = classes.reshape(batch)
        ...
        for block1, block2, attn, upsample in self.ups:
            ...
            hid = h.pop()
            if hid.shape != x.shape:
                diffX = hid.shape[-1] - x.shape[-1]
                x = F.pad(x, [0, diffX])
